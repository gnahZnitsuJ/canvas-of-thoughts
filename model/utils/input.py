...
def find_target(t, training_set=[], testing_set=[], strict=False, vocab=[]):
	# training time length
	training_time = len(training_set)*mp.tr_impression
	# testing time length
	# testing_time = (len(testing_set)-1)*mp.tr_impression
	...
	if t <= training_time:
		...
		if strict == False:
			...
			# strict case: replacing unknown words with unknown token
			# should find targets in parallel to context (strict case) (I forgot what I meant by this)
			return WordsToSPAVocab(mp.unknown_token if x not in vocab else x for x in pair[1])[0] 
